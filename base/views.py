...
def jobsPage(request):
    # Add your logic here (e.g., querying jobs from a database)
...
def registerPage(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Capture the selected user type from the form (Company or Job Finder)
            user_type = request.POST.get('user_type')

            # Save the user after form validation
            user = form.save(commit=False)
            user.username = user.username.lower()

            # Assign user type based on dropdown selection
            if user_type == 'company':
                user.user_type = 'company'
            elif user_type == 'job_finder':
                user.user_type = 'job_finder'
